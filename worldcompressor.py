...
with open("blocksfull.txt", "w") as f:
  f.write("")
with open("panesfull.txt", "w") as f:
  f.write("")
      ...
      for x in range(16):
        for y in range(256):
          for z in range(16):
            ...
            if (float(blocktypes.count(95)) / len(blocktypes) > 0.5): blocks.append([x + xchunk * 16, y, z + zchunk * 16])
            elif (float(blocktypes.count(160)) / len(blocktypes) > 0.5): panes.append([x + xchunk * 16, y, z + zchunk * 16])
